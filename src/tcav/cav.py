...
class CAV(object):
    ...
    def default_hparams():
        ...
        """
        return tf.contrib.training.HParams(model_type="linear", alpha=0.01)
    ...
    def _create_cav_training_set(concepts, bottleneck, acts):
        ...
        for i, concept in enumerate(concepts):
            x.extend(
                acts[concept][bottleneck][:min_data_points].reshape(min_data_points, -1)
            )
    ...
    def train(self, acts):
        ...
        if self.hparams.model_type == "linear":
            lm = linear_model.SGDClassifier(
                alpha=self.hparams.alpha, tol=1e-3, max_iter=1000
            )
...
def get_or_train_cav(
    ...
    tf.logging.info(
        "Training CAV {} - {} alpha {}".format(concepts, bottleneck, cav_hparams.alpha)
    )
